...
public class InterviewController {
    ...
    @PostMapping
    public ResponseEntity<InterviewResponse> startInterview(@RequestBody StartInterviewRequestDTO request) {
        Users user = authUserService.getAuthenticatedUser()
                .orElseThrow(() -> new UsernameNotFoundException("Authenticated user not found"));

        InterviewResponse response = interviewService.startInterview(user.getUsername(), request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @GetMapping("/{id}")
    public ResponseEntity<InterviewResponse> getInterviewById(@PathVariable ObjectId id) {
        InterviewResponse response = interviewService.getInterviewById(id);
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{id}/complete")
    public ResponseEntity<InterviewResponse> completeInterview(@PathVariable ObjectId id){
        Users user = authUserService.getAuthenticatedUser()
                .orElseThrow(()-> new UsernameNotFoundException("Authenticated user not found"));
        InterviewResponse response = interviewService.completeInterview(id, user);
        return ResponseEntity.ok(response);
    }
}
